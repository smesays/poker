# Code      : /result/network.py
# Written by: LIU, Jiun Ee (G); TANG, Chit Long (Steven)
# Purpose   : Train neural network model to classify player styles
# Notes     : We trained using 3 versions of data:
#               1. data with all 8 styles (with hole cards information)
#               2. data with only the 4 more meaningful styles (with hole cards information)
#               3. data with only the 4 more meaningful styles (without hole cards information)
...
train_set0 = PokerDataset('../log/tourney_log20180222.txt.gz', mask = 0)
test_set0 = PokerDataset('../log/tourney_log20180222_10k.txt.gz', mask = 0)
